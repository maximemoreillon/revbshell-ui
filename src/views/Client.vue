  ...
  <div class="terminal_view">
    <template v-if="client">
      <div class="target_info">
        <div class="">
          <span class="mdi mdi-target" /> {{ client.username }}
        </div>
      ...
      <div class="terminal" ref="terminal" v-on:click="focus_input()">
        <div
          class=""
          v-for="(entry, index) in client.history"
          v-bind:key="index"
        >
          <div class="command">
...
export default {
  ...
  mounted() {
    ...
    setInterval(() => {
      ...
      this.time_difference = Math.round(
        (new Date() - new Date(this.client.last_seen)) / 1000 + 17
      )
    }, 1000)
  },
  methods: {
  ...
  computed: {
    client() {
      return this.$store.state.clients.find(
        (c) => c.username === this.$route.params.username
      )
    },
  },
}
